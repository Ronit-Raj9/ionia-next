...
export const fetchTest = createAsyncThunk(
  "test/fetchTest",
  async (paperId: string, { rejectWithValue, getState }) => {
    try {
      // Direct API call with minimal code
      const API_BASE_URL = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000/api/v1';
      
      // Check if we're in a mock test context by looking at the URL path
      const isMockTest = window.location.pathname.includes('/mock-test/');
      let apiUrl = '';
      
      if (isMockTest) {
        // Extract exam type from URL
        const urlParts = window.location.pathname.split('/');
        const examTypeIndex = urlParts.findIndex(part => part === 'exam') + 1;
        const examType = urlParts[examTypeIndex] || '';
        
        // Use public route for mock tests
        apiUrl = `${API_BASE_URL}/tests/mock/${examType}/${paperId}/attempt`;
      } else {
        // Use authenticated route for other tests
        apiUrl = `${API_BASE_URL}/tests/${paperId}/attempt`;
      }
