...
export const GoogleLoginButton: React.FC<GoogleLoginButtonProps> = ({
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      await authAPI.googleLogin(returnUrl);
      // The page will redirect to Google OAuth, so we don't need to handle success here
  ...
  const getVariantClasses = () => {
    switch (variant) {
      ...
      case 'ghost':
        return 'bg-transparent text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700';
      default:
  ...
  return (
    <motion.button
      ...
      className={`
        inline-flex items-center justify-center space-x-3 rounded-lg font-medium
