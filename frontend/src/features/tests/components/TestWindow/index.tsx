...
import type { Test } from '@/features/tests/store/testStore';
...
const TestWindow: React.FC<TestWindowProps> = ({ examType, paperId, subject }) => {
  ...
  useEffect(() => {
    if (currentTest && activeQuestion >= 0) {
      startQuestionTracking(activeQuestion.toString());
    }
  ...
  const handleNavigateToQuestion = (index: number) => {
    startQuestionTracking(index.toString());
  ...
  const handleQuestionClick = useCallback((questionIndex: number) => {
    ...
    startQuestionTracking(questionIndex.toString());
  ...
  const handleNext = useCallback(() => {
    if (currentTest && activeQuestion < currentTest.questions.length - 1) {
      ...
      startQuestionTracking((activeQuestion + 1).toString());
  ...
  const handleSaveAndMark = useCallback(() => {
    ...
    toggleMarkQuestion(activeQuestion);
    logTestMetadata();
  }, [answerQuestion, activeQuestion, currentTest, toggleMarkQuestion, logTestMetadata]);
    ...
    try {
      ...
      if (responseData.data && responseData.data.attemptId) {
        ...
        try {
          ...
          setLoading(true);
          const rawAnalysisData = await getTestAnalysis(responseData.data.attemptId, paperId);
          
          if (rawAnalysisData) {
            // Transform the raw data to the expected format
            const transformedData = transformAnalysisData(rawAnalysisData);
    ...
    if (!isTestCompleted) {
      const timer = setInterval(() => {
        if (currentQuestionId !== null) {
          getQuestionTime(currentQuestionId);
        }
      }, 1000);
