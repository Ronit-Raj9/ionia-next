...
class QdrantMemory:
    """Qdrant-based memory store for knowledge management."""
    ...
    def _init_qdrant(self):
        ...
        try:
            QdrantClient = self.deps['qdrant_client']
            Distance = self.deps['qdrant_distance']
            VectorParams = self.deps['qdrant_vector_params']
            
            # Determine connection mode
            url = self.vector_store_config.get("url")
            api_key = self.vector_store_config.get("api_key")
            host = self.vector_store_config.get("host")
            port = self.vector_store_config.get("port", 6333)
            path = self.vector_store_config.get("path")
            ...
            self.collection_name = self.vector_store_config.get("collection_name", "knowledge_base")
            ...
            try:
                ...
                self.client.create_collection(
                    ...
                    vectors_config=VectorParams(
                        size=self._get_embedding_dimensions(self.embedding_model_name),
    ...
    def get_all(self, user_id=None, agent_id=None, run_id=None):
        ...
        try:
            ...
            logging.error(f"Error getting all memories from Qdrant: {e}")
            return []
    ...
    def get(self, memory_id):
        ...
        try:
            ...
            logging.error(f"Error getting memory from Qdrant: {e}")
            return None
    ...
    def update(self, memory_id, data):
        ...
        try:
            ...
            # Update content
            point = existing[0]
            point.payload["content"] = data
            point.payload["updated_at"] = datetime.utcnow().isoformat()
            ...
            logging.error(f"Error updating memory in Qdrant: {e}")
            return False
    ...
    def delete(self, memory_id):
        ...
        try:
            ...
            logging.error(f"Error deleting memory from Qdrant: {e}")
            return False
    ...
    def delete_all(self, user_id=None, agent_id=None, run_id=None):
        ...
        try:
            ...
            logging.error(f"Error deleting memories from Qdrant: {e}")
            return False
...
class Knowledge:
    ...
    def _deps(self):
        try:
            ...
            return {
                ...
                'qdrant_vector_params': VectorParams,
                'markdown': MarkItDown()
            }
        except ImportError:
            raise ImportError(
        ...
        if self._config:
            ...
            if "vector_store" in self._config:
                if "provider" in self._config["vector_store"]:
                    ...
                    # Special handling for MongoDB vector store (keep for backward compatibility)
    ...
    def memory(self):
        ...
        try:
            return QdrantMemory(self.config, self._deps)
        except Exception as e:
