...
import { PersonalizationRequest, PersonalizationResponse, GradingRequest, GradingResponse } from './groq';
...
export async function personalizeAssignmentFallback(
...
Return ONLY valid JSON in this format:
{
  ...
  "variations": "Description of adaptations made (e.g., 'Added visual diagrams for visual learner, included remedial fractions practice')"
    ...
    throw new Error(`Failed to personalize assignment with OpenAI: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}

export async function gradeSubmissionFallback(
...
Return ONLY valid JSON in this format:
{
  ...
  "errors": ["Arithmetic error in step 3: 2+3=6 should be 2+3=5", "Missing unit in final answer"]
    ...
    throw new Error(`Failed to grade submission with OpenAI: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
