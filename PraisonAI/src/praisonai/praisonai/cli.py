...
class PraisonAI:
    ...
    def main(self):
        ...
        if args.auto or self.auto:
            print("[red]ERROR: Auto mode is no longer supported. Please use direct agent configuration.[/red]")
            sys.exit(1)
        elif args.init or self.init:
            print("[red]ERROR: Init mode is no longer supported. Please use direct agent configuration.[/red]")
            sys.exit(1)
    ...
    def parse_args(self):
        ...
        if not args.command and (args.init or args.auto):
            if not PRAISONAI_AVAILABLE:
                print("[red]ERROR: PraisonAI Agents is not installed. Please install it with:[/red]")
                print("\npip install praisonaiagents\n")  
                sys.exit(1)
    ...
    def create_gradio_interface(self):
        ...
        """
        print("[red]ERROR: Gradio interface with auto-generation is no longer supported. Please use direct agent configuration.[/red]")
        sys.exit(1)
